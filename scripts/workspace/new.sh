...
DEFAULT_KEYNAME="devbox_${WS_NAME}_ed25519"
prompt KEYNAME "SSH key name under devbox/keys/ (without .pub)" "$DEFAULT_KEYNAME"
PUBKEY_FILE="${KEYNAME}.pub"
PRIVKEY_FILE="$KEYNAME"
need_file "$KEYS_DIR/$PUBKEY_FILE"
need_file "$KEYS_DIR/$PRIVKEY_FILE"
...
sed \
  ...
  -e "s/__PRIVKEY_FILE__/${PRIVKEY_FILE//\//\\/}/g" \
...
# Write ws.env (metadata for shared helper)
cat > "$WS_DIR/ws.env" <<EOF
WS_NAME=$WS_NAME
SSH_PORT=$SSH_PORT
